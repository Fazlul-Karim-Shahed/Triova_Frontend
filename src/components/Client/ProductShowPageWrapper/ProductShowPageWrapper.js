...
export default function ProductShowPageWrapper({ initialProducts, initialSubCategory }) {
    ...
    useEffect(() => {
        const fetchFiltered = async () => {
            ...
            const res = await getAllProductApi(null, query); // You must implement this API call
            if (!res.error) {
                setLoading(false);
    ...
    return (
        <div className="mx-auto px-4 py-10 bg-gradient-to-b from-white/30 to-white/10 backdrop-blur-md">
            <div className="border-y p-3 mb-8 lg:hidden rounded-lg shadow-sm bg-white/30 backdrop-blur-md">
                <ProductFilterSM products={products} categoryId={subCategory} searchParams={Object.fromEntries(searchParams.entries())} />
            </div>
                                ...
                                products.map((item, index) => (
                                    <div key={index} className="flex flex-col backdrop-blur-xl bg-white/20 border-2 rounded-3xl p-2 transition-transform transform hover:scale-[1.02] hover:shadow-lg">
                                        <Link href={`/products/${encodeURIComponent(item.name)}`} className="relative rounded-2xl overflow-hidden group block">
                                            ...
                                            />
                                            {item.discount > 0 && (
                                                <span className="absolute top-2 left-2 bg-black/70 text-white text-xs px-3 py-1 rounded-full font-semibold">{item.discount}% OFF</span>
                                            )}
                                        </Link>
